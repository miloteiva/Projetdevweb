...
<head>
    <meta charset="utf-8">
    <meta name="csrf-token" content="<?php echo htmlspecialchars($_SESSION['csrf_token'] ?? (bin2hex(random_bytes(16)))); ?>">
...
<body>
    ...
    <main class="compte-container">
        ...
        <section class="section">
            ...
            <div style="margin-top: 25px;">
                <button id="btn-edit-profile" class="btn-action">✏️ Modifier mon profil</button>
            </div>
            ...
            <?php else: ?>
                ...
                <div class="commande-card">
                    <div class="cmd-info">
                        ...
                        <?php if (isset($cmd['preparation_immediate']) && !$cmd['preparation_immediate'] && !empty($cmd['date_prevue'])): ?>
                            <p style="color: var(--gold-color); font-size: 0.85rem;">
                                🕒 Prévu pour le <?= htmlspecialchars($cmd['date_prevue']) ?> à <?= htmlspecialchars($cmd['heure_prevue']) ?>
                            </p>
                        <?php endif; ?>
                    ...
                    <div class="commande-actions">
                        ...
                        <?php endif; ?>
                        <?php if (in_array($cmd['statut'], ['Livrée','Payée','En préparation','Prête','En livraison'])): ?>
                            <button class="btn-action btn-small btn-recommande" 
                                    data-cmd-id="<?= $cmd['id'] ?>"
                                    style="font-size:0.7rem;padding:5px 10px;">♻ Recommander</button>
                        <?php endif; ?>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        document.querySelectorAll('.btn-recommande').forEach(btn => {
            btn.addEventListener('click', async () => {
                if (!confirm('Ajouter tous les articles de cette commande au panier ?')) return;
                btn.disabled = true; btn.textContent = '...';
                const result = await apiCall('api/recommande_commande.php', {
                    method: 'POST',
                    headers: {'Content-Type':'application/json'},
                    body: JSON.stringify({ order_id: parseInt(btn.dataset.cmdId, 10) })
                });
                if (result.success) {
                    notify(result.message, 'success');
                    setTimeout(() => window.location.href = 'panier.php', 1200);
                } else {
