...
// Mode : libre (avis général) ou lié à une commande spécifique
...
// Vérifications uniquement si on note une commande spécifique
$erreur_acces = null;
if (!$mode_libre) {
    if (!$commande) {
...
<head>
    ...
    <title>Noter mon expérience - Les Arcades</title>
    ...
    <style>
        ...
        .mode-badge { display: inline-block; padding: 4px 12px; border-radius: 14px; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 15px; }
        .mode-libre { background: rgba(230, 140, 124, 0.15); color: var(--gold-color); }
        .mode-commande { background: rgba(76, 175, 80, 0.2); color: #A5D6A7; }
...
<body>
    ...
    <main class="notation-container">
        <section class="rating-card" id="rating-card">
            ...
            <?php if ($erreur_acces): ?>
                ...
                <a href="notation.php" style="color:var(--gold-color); display:inline-block; margin-top:15px;">→ Laisser un avis général</a><br>
                <a href="moncompte.php" class="btn-retour">← Retour à mon compte</a>

            <?php else: ?>
                <?php if ($mode_libre): ?>
                    <span class="mode-badge mode-libre">Avis général</span>
                    <h1 class="main-title">Votre Avis</h1>
                    <p>Partagez votre expérience chez Les Arcades !</p>
                    <div class="order-info">Vous pouvez laisser autant d'avis que vous le souhaitez</div>
                <?php else: ?>
                    <span class="mode-badge mode-commande">Commande #<?= $commande['id'] ?></span>
                    <h1 class="main-title">Votre Avis</h1>
                ...
                <form id="rating-form" onsubmit="return false;">
                    <input type="hidden" id="order-id" value="<?= $mode_libre ? 0 : $commande['id'] ?>">
                    ...
                    <div class="rating-group">
                        <h3 style="color:var(--gold-color);"><?= $mode_libre ? 'Service en général' : 'Service de livraison' ?></h3>
                        <div class="stars">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const isModeLibre = <?= $mode_libre ? 'true' : 'false' ?>;
        ...
        submitBtn.addEventListener('click', async () => {
            ...
            if (result.success) {
                ...
                const retourBtn = isModeLibre
                    ? '<a href="notation.php" class="btn-retour" style="margin-right:15px;">Laisser un autre avis</a><a href="restaurant.php" class="btn-retour">Retour à l\'accueil</a>'
